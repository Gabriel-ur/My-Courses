...
#existem palavras que ajudam no tratamento de erros

try: #verifica se vai ocorrer algum erro nesse bloco de código; é necessário o 'except' pra funcionar
    n = int(input('Número inteiro: '))
except Exception as erro: #mostra o informações sobre o erro ocorrido
    print(f'O erro encontrado foi: {erro}')
except ValueError: #foi especificado (opicional) qual o tipo de erro deseja analisar 
                   #aqui ele não funciona por ja ter o 'Exception' em cima
    print('O que você digitou não é um número inteiro')
except: #faz algo caso um erro ocorra
    print('Sei lá o que rolou')
else: #esse 'else' serve pra fazer essa ação debaixo apenas se não der problema
    print(f'O número que digitou é {n}! :)')
finally: #faz isso independentemente do 'try' e 'except'
